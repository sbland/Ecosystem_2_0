	...
	public class IllogikaDrawer : PropertyDrawer {
		...
		protected System.Object GetFieldOrPropertyValue(Component component, string fieldName)
		{
			if(component.GetType().GetField(fieldName) != null)
				return component.GetType().GetField(fieldName).GetValue(component);
			else if(component.GetType().GetProperty(fieldName) != null)
				return component.GetType().GetProperty(fieldName).GetValue(component, null);
			else
		...
		{
			MethodInfo buttonFunction = type.GetMethod(methodName, BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance);
			if (buttonFunction == null)
			{
